...
- Leave balance on rehire: configurable per company. Options:
  - Reset to zero (default)
  - Carry over remaining balance from last employment
...
- A scheduled Edge Function runs at 23:59 IST every day.
- For every employee with a `check_in_time` but no `check_out_time` for today: set `check_out_time = 23:59:00`, set `status = 'incomplete'`.
...
- Logic:
  ...
  2. If date is in `holidays` table (and not optional/unselected): status = `holiday`.
  3. If date is a weekly-off day per the employee's shift: status = `weekly_off`.
  4. If `check_in_time` IS NULL: status = `absent`.
  5. If `check_in_time` IS NOT NULL and `check_out_time` IS NULL (after auto-checkout): status = `incomplete`.
  6. If total_hours < (shift_hours / 2): status = `half_day`.
  7. If `check_in_time` IS NOT NULL and `check_out_time` IS NOT NULL and total_hours >= (shift_hours / 2): status = `present`.
  8. If employee logged WFH (explicit action): status = `work_from_home` (overrides absent).
...
- Employee can request regularization for records within the past N calendar days (default: 7 days). N is a system config value.
- Requests beyond the window are blocked: "Regularization window is [N] days. This date is outside the allowed window."
- One pending regularization request per attendance record at a time. If a request exists for a date, block new request until existing one is resolved.
...
- Working days = count of days in `[from_date, to_date]` range that are not:
  - In `holidays` table for applicable state/type (non-optional, or employee has opted-in to optional holiday)
...
- If a `leave_applications` row has `status = 'pending'` and `applied_at < now() - 2 business days`:
  - Set `escalated_to = Owner.id`
  - Set `escalated_at = now()`
  - Send email + in-app notification to Owner: "Leave request from [employee] has been pending for 2 business days."
- Business days exclude holidays and weekly-offs (use the company's holiday calendar).

### BR-LVE-07 — HR on Leave Routing
- When a leave application is submitted: check if the employee's `reporting_manager_id` (HR) has an active approved leave for the same date range.
- If HR is on leave: route directly to Owner (`reviewed_by = Owner.id`). Set this at submission time.
...
- **Approved future application:** Employee submits cancellation request. `status` does not change to `cancelled` yet. HR/Owner sees a "Cancellation requested" flag. HR/Owner confirms → `status = 'cancelled'`, balance restored. HR/Owner rejects cancellation → status remains `approved`.
...
- For each employee × leave type:
  ...
  2. Set `opening_balance = carry_forward` for the new year row.
...
- For each `leave_balances` row where `carry_forward_expiry IS NOT NULL`:
  ...
  - If `carry_forward_expiry = today`: lapse the carry-forward portion. Reduce `opening_balance` by the carry-forward amount that is expiring. Log to `audit_logs`.

### BR-LVE-12 — Comp-Off Expiry
- Default expiry: `worked_date + 60 days`. Configurable per company (store as app config).
...
| Leave SLA breach (2 business days) | Owner | Email |
...
- If the action is triggered by a scheduled Edge Function (no user context): `actor_id = NULL`, `actor_role = NULL`. Include `actor_system_function` as a text label (e.g. `auto_checkout`, `year_end_rollover`). Add this column to `audit_logs`.
...
|---|---|---|
| `auto-checkout` | Daily at 23:59 IST | Mark incomplete attendance, trigger notification |
...
| `year-end-leave-rollover` | Jan 1st, 00:30 IST | Process carry-forward, create new year leave balance rows |
| `carry-forward-expiry-alert` | Daily at 09:00 IST | Send 30-day and 7-day expiry warnings |
| `carry-forward-lapse` | Daily at 00:20 IST | Lapse expired carry-forward balances |
...
| `leave-sla-escalation` | Every hour | Escalate pending leave requests past 2 business days |
...
| Comp-off expiry | worked_date + 60 days default | Edge Function on approval |
...
| Regularization window | Within last N days | Edge Function |
| Duplicate PAN/Aadhar | Block + override flow | Edge Function |
